"""
סקריפט לאיסוף מחירים מטיב טעם וקשת טעמים
מפעיל את הסקריפטים בנפרד ומאחד לקובץ JSON אחד
"""
...
CATEGORIES = {
    ...
    "הודו": ["הודו"],
    "בקר": ["בקר", "עגל", "אנטריקוט", "סינטה", "פיקניה", "שייטל", "צ'אך", "צאך", "אסאדו"],
    "בשר לבן": ["לבן", "חזיר"],
...
def scrape_chain(chain_key, chain_config):
    ...
    print(f"\n{'='*50}")
    print(f"Scraping {chain_config['name']}...")
    ...
    # קח קובץ אחד לכל סניף (הראשון)
    # בפועל כדאי לקחת סניף ספציפי - כאן נקח את הראשון
    filename = files[0]
    ...
    categorized = filter_items(items)
    total = sum(len(v) for v in categorized.values())
    print(f"  Categorized {total} relevant items")
